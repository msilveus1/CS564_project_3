...
const int BTreeIndex::split_1(void *&childNode,int isLeaf, PageId &newID,PageId &currentId,int &keyValue, RecordId rid,PageId& childPageId_1,PageId& childPageId_2){
	try{
		if(isLeaf){
				...
				for(int i = 0; i < height; i++){				
					...
					if(checkOccupancy(currentNode->keyArray,INTARRAYNONLEAFSIZE,0,currentNode->pageNoArray,NULL)){
						if(i + 1 == height){
							...
							currentkey = split_1(currentNode,0,newID, tempVal,*tempkey,NULL,*child_Id_1_1,*child_Id_1_2);
